/* app.js v266 FINAL - COMPLEET - Proxy geeft 5 artikelen met datum */
const BRONNEN=[
{id:'De Stentor',name:'De Stentor',sub:'regionaal'},
{id:'Gemeente Ommen',name:'Gemeente Ommen',sub:'officieel'},
{id:'Natuurlijk Ommen',name:'Natuurlijk Ommen',sub:'evenementen'},
{id:'Ommen City',name:'Ommen City',sub:'lokaal'},
{id:'OudOmmen',name:'OudOmmen',sub:'historie'},
{id:'RondOmmen',name:'RondOmmen',sub:'lokaal'},
{id:'RTV Oost',name:'RTV Oost',sub:'Overijssel'},
{id:'RTV Vechtdal',name:'RTV Vechtdal',sub:'Vechtdal'},
{id:'Vechtdal Centraal',name:'Vechtdal Centraal',sub:'112'},
];
const BRON_URLS={
...
function setupHeader(){
  ...
  fh.addEventListener('click',e=>{
    ...
    if(e.target.id==='btn-all'||e.target.closest('#btn-all')){
      ...
      saveState();renderFilters();refreshNews();return;
    }
    const p=document.getElementById('source-panel');
    if(p.classList.contains('open')){p.classList.remove('open');fh.classList.remove('open');document.body.classList.remove('panel-open');}
    else{fh.classList.add('open');p.classList.add('open');document.body.classList.add('panel-open');}
  });
}

async function fetchWithTimeout(url,ms){
  const c=new AbortController();const to=setTimeout(()=>c.abort(),ms);
  try{const r=await fetch(url,{cache:'no-store',signal:c.signal});clearTimeout(to);return await r.text();}catch(e){clearTimeout(to);throw e;}
}

async function fetchRSS(url){
  try{
    const txt=await fetchWithTimeout(`https://api.rss2json.com/v1/api.json?rss_url=${encodeURIComponent(url)}&t=${Date.now()}`,3000);
    const j=JSON.parse(txt);
    if(j.status==='ok'&&j.items&&j.items.length>0){
      return j.items.slice(0,12).map(it=>({title:it.title,link:it.link,pubDate:new Date(it.pubDate),description:(it.description||'').replace(/<[^>]*>/g,'').slice(0,120)}));
    }
  }catch{}
  return [];
}

// DEZE geeft 5 artikelen met datum uit page source
async function fetchOost(url){
  try{
    const proxyUrl=`${WORKER}/proxy?url=${encodeURIComponent(url)}&t=${Date.now()}`;
    const html=await fetchWithTimeout(proxyUrl,8000);
    if(!html||html.length<1000) return [];
    let items=[];let m;
    const re1=/"publishedAt"\s*:\s*"([^"]+)"[\s\S]{0,800}?"title"\s*:\s*"([^"]+)"[\s\S]{0,800}?"url"\s*:\s*"([^"]+)"/gi;
    const re2=/"url"\s*:\s*"(\/nieuws\/vechtdal\/[^"]+)"[\s\S]{0,800}?"title"\s*:\s*"([^"]+)"[\s\S]{0,800}?"publishedAt"\s*:\s*"([^"]+)"/gi;
    while((m=re1.exec(html))!==null && items.length<5){
      let d=m[1], t=m[2].replace(/\\"/g,'"'), u=m[3];
      if(!u.startsWith('http')) u='https://www.rtvoost.nl'+u;
      items.push({title:t, link:u, pubDate:new Date(d), description:t, source:'RTV Oost', id:'RTV Oost'});
    }
    while((m=re2.exec(html))!==null && items.length<5){
      if(items.find(x=>x.link.includes(m[1]))) continue;
      let u=m[1], t=m[2].replace(/\\"/g,'"'), d=m[3];
      if(!u.startsWith('http')) u='https://www.rtvoost.nl'+u;
      items.push({title:t, link:u, pubDate:new Date(d), description:t, source:'RTV Oost', id:'RTV Oost'});
    }
    return items;
  }catch(e){console.log('Oost error',e);return [];}
}
...
async function loadBron(b){
  try{
    ...
    if(arts.length>0){
      allArticles=allArticles.filter(x=>x.id!==b.id).concat(arts.map(a=>({...a,source:b.name,id:b.id})));
    }
  }catch{}loaded++;
}
...
function renderArticles(){
  ...
  if(header) header.textContent=`${activeIds.length} v/d ${BRONNEN.length} bronnen`;
  c.innerHTML=`<div class="articles-count">${f.length} artikelen - ${loaded} v/d ${activeIds.length} bronnen geladen (filter: ${activeIds.join(', ')})</div>`
  +f.map(a=>{
    const d=a.pubDate?`${a.pubDate.getDate()}-${a.pubDate.getMonth()+1} ${a.pubDate.getHours()}:${String(a.pubDate.getMinutes()).padStart(2,'0')}`:'';
    return `<div class="article"><h2><a href="${a.link}" target="_blank">${a.title}</a></h2><small>${a.source} - ${d}</small><div>${a.description||''}</div></div>`;
  }).join('')
  +(f.length===0?`<div class="article">Geen artikelen voor ${activeIds.join(', ')} gevonden.</div>`:'');
}
