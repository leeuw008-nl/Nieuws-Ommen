/* service-worker v237-PERF - FIX CACHE + SNEL + RTV OOST BLIJFT WERKEN
 * - Correcte icon paden
 * - Nooit RSS/HTML cachen als assets (alleen shell)
 * - Cache first voor shell, network first voor app.js met 3s timeout
 */
const CACHE_NAME='ommen-v237-perf';
const STATIC_ASSETS=[
  './',
  './index.html',
  './styles.css',
...
self.addEventListener('fetch', e=>{
  ...
  // Nooit proxy/worker/rss cachen in SW
  if(u.hostname.includes('workers.dev') || u.hostname.includes('allorigins') || u.hostname.includes('rss2json') || u.pathname.includes('/proxy')){
    return; // laat browser direct fetchen
  }
  // Voor app.js / push.js / article-focus.js: network first met snelle fallback, max 3s
  if(u.pathname.includes('app.js')||u.pathname.includes('push.js')||u.pathname.includes('article-focus.js')||u.pathname.includes('styles.css')){
    e.respondWith(
      Promise.race([
...
self.addEventListener('message', e=>{if(e.data && e.data.type==='SET_FILTERS'){console.log('[v237-perf] Filters:', e.data.sources);}});
