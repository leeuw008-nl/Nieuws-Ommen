...
function searchNews() {
    ...
    // FIX BACKUP: niks aangevinkt = niks tonen (zoals vroeger)
    if(gekozenBronnen.length===0){
      renderArticles([]);
      const c = document.getElementById("news-container");
      if(c) c.innerHTML = "<p>Selecteer minimaal één bron bij 'Bronnen'.</p>";
      return;
    }
...
window.addEventListener("DOMContentLoaded", function() {
    setupSearch(); setupSources(); ensureBanner(); loadNews(false); // push disabled in backup
});
