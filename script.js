...
const CACHE_KEY = 'ommen_cache_v120';
...
function sanitizeFinal(text){
    if(!text) return " [...]";
    let d = String(text);
    d = d.replace(/Home Vechtdal TV[\s\S]{0,800}?Stichting RTV Vechtdal/gi, ' ');
    d = d.replace(/Vechtdal TVNieuwsVideo/gi, ' ');
    d = d.replace(/VechtdalNext/gi, ' ');
    d = d.replace(/\[\s*…\s*\]/g, ' __ELL__ ');
    d = d.replace(/\[\s*\.\.\.\s*\]/g, ' __ELL__ ');
    d = d.replace(/\[\s*\]/g, ' ');
    d = d.replace(/&hellip;/gi, ' __ELL__ ');
    d = d.replace(/…/g, ' __ELL__ ');
    d = d.replace(/\s*\.\.\.\s*/g, ' __ELL__ ');
    d = d.replace(/(\s*__ELL__\s*){2,}/g, ' __ELL__ ');
    d = d.replace(/\s+/g, ' ').trim();
    d = d.replace(/__ELL__/g, '[...]');
    d = d.replace(/(\[\.\.\.\]\s*){2,}/g, '[...] ');
    d = d.replace(/\s*\[\.\.\.\]\s*\[\.\.\.\]\s*/g, ' [...] ');
    ...
    if(d.includes('[...]')){
        let parts = d.split('[...]');
        d = parts[0].trim() + ' [...]';
        if(parts[0].includes('</p>')) d = parts[0].replace(/<\/p>\s*$/i,' [...]</p>');
    }
    if(!d.includes('[...]')){
...
function cleanHTML(html, maxLength=MAX_DESC){
    if(!html) return "";
    if(html.indexOf("Home Vechtdal TV")!==-1 || html.indexOf("Vechtdal TVNieuwsVideo")!==-1){
        html = html.replace(/Home Vechtdal TV[\s\S]*?Stichting RTV Vechtdal[\s\S]*/gi, ' ');
        if(html.trim().length<30) return "";
    }
    ...
    doc.querySelectorAll("script, style, iframe, nav, header").forEach(el=>el.remove());
    let plain=doc.body.innerText.replace(/\s+/g," ").trim();
    if(plain.indexOf("Home Vechtdal TV")!==-1) return "";
...
async function fetchRTVVechtdalNieuws(){
    ...
    try{
        ...
        return data.map(item=>{
            let raw = (item.excerpt?.rendered||item.content?.rendered||"").replace(/Home Vechtdal TV[\s\S]*?Stichting RTV Vechtdal/gi,'');
            let desc = raw ? cleanHTML(raw, MAX_DESC) : "";
            if(desc.indexOf("Home Vechtdal TV")!==-1) desc = "";
